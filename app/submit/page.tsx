...
export default function SubmitMemory() {
  ...
  const handleFiles = (fileList: FileList | File[]) => {
    ...
    if (incoming.length === 0) {
      setError("Please choose one or more image files for the photo upload.");
      return;
    }

    const oversized = incoming.find((file) => file.size > MAX_MB * 1024 * 1024);
    if (oversized) {
      setError(`"${oversized.name}" is larger than ${MAX_MB}MB.`);
      return;
    }

    const nextFiles = [...selectedFiles, ...incoming].slice(0, MAX_PHOTOS);
    setSelectedFiles(nextFiles);
    setPreviews((prev) => [...prev, ...incoming.slice(0, MAX_PHOTOS - prev.length).map((file) => URL.createObjectURL(file))]);
    setError(null);
  };
      ...
      if (selectedFiles.length > 0) {
        ...
        const uploadedUrls = await Promise.all(
          selectedFiles.map(async (file) => {
            ...
            const path = `${Date.now()}-${Math.random().toString(36).slice(2)}-${file.name.replace(/\s+/g, "-")}.${ext}`;
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 py-12">
      ...
      <div className="text-center mb-8 relative">
        <div className="absolute -top-2 left-1/2 -translate-x-1/2 text-4xl select-none pointer-events-none opacity-20">🍯</div>
        ...
        <p className="text-slate-500 text-sm mt-2 max-w-xs mx-auto">
          Leave a message, share a clip, or attach a photo. She&apos;ll see it on her birthday! 🐝
        </p>
      </div>
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl border border-amber-50 p-6 md:p-8 space-y-5">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {TAGS.map(t => (
              <button
                key={t}
                onClick={() => setRelationship(t === relationship ? "" : t)}
        ...
        <div>
          ...
          <input
            ...
            className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-amber-300 transition"
          />
          <p className="text-xs text-slate-400 mt-1.5">Paste a Twitch clip, YouTube, or TikTok link — it&apos;ll embed on her board.</p>
        </div>

        {/* Photo upload */}
        <div>
          <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1.5">
            Photos <span className="font-normal text-slate-400 normal-case">(optional, up to 6)</span>
          </label>
          <div
            onClick={() => fileRef.current?.click()}
            onDrop={e => { e.preventDefault(); if (e.dataTransfer.files.length) handleFiles(e.dataTransfer.files); }}
            onDragOver={e => e.preventDefault()}
            className="relative border-2 border-dashed border-amber-200 rounded-xl p-5 text-center cursor-pointer hover:border-amber-400 hover:bg-amber-50/50 transition-all"
          >
            ...
            {previews.length > 0 ? (
              <div className="grid grid-cols-2 gap-3">
                {previews.map((src, index) => (
                  <img key={src} src={src} alt={`Preview ${index + 1}`} className="h-24 w-full rounded-xl object-cover" />
                ))}
              </div>
            ) : (
              <>
                <div className="text-3xl mb-1">📷</div>
                <p className="text-sm text-slate-400">Click or drag up to 6 photos here</p>
              </>
            )}
          </div>
          {previews.length > 0 && (
        ...
        <button
          ...
          className={`w-full rounded-full py-4 font-bold text-white text-sm tracking-wide transition-all ${
            isSubmitting
              ...
              : "bg-linear-to-r from-amber-500 to-yellow-500 hover:from-amber-600 hover:to-yellow-600 hover:shadow-lg active:scale-95 cursor-pointer"
          }`}
        >
          {isSubmitting ? "Sending to the hive..." : "Send my memory 🍯"}
        </button>
      </div>
    </div>
  );
